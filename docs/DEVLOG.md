...
- [ ] **UI 拉皮：以 `Phobos-UI` 雜誌風設計改寫前端**（研究已完成，見 `plan/ui-reskin-2026-08-12.md`；**待 batu 拍板四題後才開票**到 `.scratch/ui-reskin/issues/`）。
  ...
  - **⏳ 唯一剩下的待決：sparkline 走勢**（plan §5.5）。它是球員頁那張「近七場狀態走勢」小折線（`components/sparkline.tsx`，57 行 SVG、無軸無刻度）。**兩個坑**：餵它的 0-100「狀態分數」不是任何棒球數據、StatsAPI 與我們 DB 都推不出來，是設計為填圖編造的（要上真值得先定義算法，那才是工作量）；且 sparkline **自我正規化**（`range = max - min || 1`）⇒ 只表達形狀不表達幅度，58→80 與 .290→.295 畫出來一樣。**三條路**：A 整區不做（最省）／**B 建議——改畫季內累積走勢（打者 season-to-date OPS、投手 ERA），資料現成於 `game_*_lines`、語意誠實，但圖上必須標明指標名與起訖值**／C 定義狀態分數（等於開小 spec，且會成為全站唯一沒有出處的數字，不建議）。**不比照媒體用 mock**——媒體是「等資料源」，sparkline 是「等定義」，mock 一個編造分數等於把待決問題畫進 UI。
