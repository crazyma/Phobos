...
- **告警語意**：source 正常完成時可回傳結構化 warning；批次將其依 source 寫入 `sync_runs.detail.sources_warnings`。warning 純供稽核，**不影響** `success`／`partial`／`failed`（`derive_status` 只依來源是否失敗判定）。既有無 warning 的 detail 保持原形狀，讀取端須容忍沒有 `sources_warnings`。
