...
- **隊名顯示規則**（spec-01 C.2）：大聯盟＝中文暱稱（「水手」）；小聯盟＝母隊中文名＋（原名）（「紅襪（Worcester Red Sox）」）。名冊列與個人頁 hero 自己印層級徽章，故隊名**不**重複帶層級（`teamDisplayName(..., { withLevel: false })`）；單獨出現隊名的地方才帶層級。密集介面（季數據表隊伍欄、首頁卡片、即將出賽對手）沿用 `abbrev`。
