...
> # ⚠️ 本票已併入票 02，不要單獨執行
>
> **2026-08-13，batu 決定**：隊徽同時出現在**名冊卡**與**球員頁 hero**，兩處都落在 `PlayerCard` 與 hero，拆成獨立一票等於讓兩個 agent 前後動同一批檔案。整張併進 `02-player-page-profile-and-moves.md` 的「隊徽」一節。
>
> 本檔保留供查閱（推導規則、fallback、素材決策都在下面，票 02 直接引用），但**不是可執行的票**。切片因此從 7 票收斂為 6 票。

**What to build:** 名冊卡與球員頁 hero 顯示所屬球隊的隊徽。小聯盟球員顯示**母球團**的隊徽，未來新增的球隊與對手自動涵蓋、不需回頭補素材。取不到時不顯示破圖，該元素直接省略。

**Blocked by:** ~~01~~ — 已併入 02。
