...
**What to build:** 讓 `game_batting_lines`／`game_pitching_lines` 不再依賴 `games` 就能顯示與排序。目前逐場表只有 `player_id / game_pk / team_id / level` 加數據欄，**沒有日期、沒有對手**，所以 `player-recent.ts:89-113` 與 `home.ts:96-137` 都必須 `innerJoin games`——而這個 join 實際只取三欄：`game_date_us`、`home_team_id`、`away_team_id`（比分／球場／狀態／系列賽一欄都沒用到）。這個 FK 也是 `games` 存著 1736 筆歷史比賽表頭的唯一理由（`DEVLOG:69`：gameLog 每筆「順手 upsert `games` 表頭…**以滿足外鍵**」）。

**決策（2026-07-29，batu）**：`games` 現在同時扮演「未來賽程表」與「歷史比賽維度表」，兩者生命週期完全相反（一個過期就該丟、一個要永久保留），混在同一張表是那 929 筆無人參照殘留的根因。拆開兩個角色。**理由是語意誠實，不是省空間**——15 人規模下 `games` 也才約 7,500 筆／1 MB。
...
- [ ] **回填既有 1782 列**：由 `games` join `game_pk` 取 `game_date_us`；`opponent_team_id` = 主客兩隊中不等於 `team_id` 的那支；`is_home` = `team_id = home_team_id`。`team_id IS NULL` 者 opponent／is_home 留 NULL。回填後對 `game_date_us` 上 NOT NULL
...
- [ ] **改查詢，移除對 `games` 的 join**（共 7 處，**TS 5 處 + Python 2 處**）
  - `lib/services/player-recent.ts:89-113`（打／投各一）：日期、對手、主客場改讀逐場表自己的欄位；`opponentOf()` 的呼叫改為直接吃 `opponent_team_id` / `is_home`
  - `lib/services/home.ts:96-105`（`getDigestDate` 打／投各一）與 `:136`（digest 當日 line）：`game_date_us` 改讀逐場表
...
- [ ] 測試：回填正確性（含 `team_id IS NULL` 與對手不在 `teams` 兩個邊界）；ETL 寫入三欄（打者／投手／二刀流同場兩列／客場）；`player-recent` 對手與主客場顯示不變；`home` digest 錨定日不變；**近況引擎五種 pattern 續綠**（`etl/tests/test_recent_form.py:180` 的 fixture 目前 `insert into games` 取日期，需改為直接在 line 上帶日期）。`cd etl && uv run pytest -q` 全綠、Node 全綠、`pnpm typecheck`、`pnpm build` 過
