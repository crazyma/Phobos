...
**Status:** done（2026-08-03，commit `43ed9f2`；A／B／C 三段皆完成，B 與 `xwoba-savant/01` 同批做掉，解除了它的 blocker）
...
- [x] 移除 `config.py:24` 的 `GAMELOG_LOOKBACK_DAYS` 與其註解
- [x] `cli.py:11` docstring 改寫：`resync --gamelog` 的定位是「回補早於當季的歷史逐場」，不再以 lookback 天數描述
- [x] `docs/spec/spec-03-etl-pipeline.md:37` 逐場成績那列，批次欄改為「morning／evening 各抓整個當季 gameLog（冪等，evening 補 morning 時未結束的場次）；歷史球季一次性 `etl backfill`」
- [x] `docs/spec/spec-00-overview.md:53` 移除 `GAMELOG_LOOKBACK_DAYS` 那列參數
- [x] **不要改 `docs/DEVLOG.md:104`／`:83`**——那在「已完成」區，正確記載當時的實作狀態，是歷史紀錄
- [x] **同理不要改已完成票裡的提及**：`.scratch/etl-pipeline/issues/04-game-lines-ingest.md:10` 與 `.scratch/etl-gamelog-refactor/issues/01-*.md:17` 也寫了 `GAMELOG_LOOKBACK_DAYS`，但兩處都是已打勾的 `[x]` 項、記錄當時的驗收條件，**是歷史紀錄不是漂移**。清點時會 grep 到它們，別順手改掉（2026-08-03 補註）
...
- [x] `docs/adr/decisions.md` §6.4 補一則決策：**Savant 改直接讀官方 CSV 匯出，不經 pybaseball**。記錄理由（依賴體積 vs 單一欄位；`csv=true` 是官方匯出參數、非 scraping；2026-07-29 實測 200／`text/csv`）與影響（ETL 不需 pybaseball 依賴）
...
- [x] 同步更新 `decisions.md:132`／`:139` 與 `spec-03:39` 的措辭
- [x] `decisions.md:37`（選 Python 的理由是「主要資料源 `pybaseball` 為 Python 套件」）補註：該前提已隨 §6.4 演變，但**選 Python 的結論不變**（psycopg + 標準庫已足夠，且 ETL 已成熟）
...
- [x] 改寫該條目措辭為「再次驗證 ADR §6.4（2026-07-23）的結論仍成立」，並指向 ADR，避免日後誤以為是兩次獨立發現
...
- **A 全部完成**：`config.py` 的 `GAMELOG_LOOKBACK_DAYS` 已刪（`grep` 全 repo 僅剩 DEVLOG 已完成區與
  兩張已完成票的歷史紀錄，正如票上交代的**刻意不動**）；`cli.py` docstring 的 `resync --gamelog`
  已改為「回補早於當季的歷史逐場」；`spec-03` 來源對照表與 `spec-00` §4 參數表都已更新。
- **B 全部完成**：`decisions.md` §6.4 的「可用來源」已改成 **MLB Stats API ＋ Savant 官方 CSV 匯出**，
  決策段落記下了「不引 pybaseball」的理由（依賴體積 vs 單一欄位、`csv=true` 是官方匯出、
  2026-07-29 實測 200／`text/csv`）與**固有限制**（leaderboard 粒度是球員×球季、`team=` 只篩名單
  不改口徑 → 換隊球季拿不到分隊 xwOBA）；`decisions.md:37` 選 Python 的理由也補了
  「pybaseball 前提已隨 §6.4 演變，但語言結論不變」；`spec-03` 措辭同步。
  票上「等 xwoba 票完成後再補」的顧慮已不存在——兩者同一批做完，ADR 記的就是實際落地的選型。
- **C 完成**：DEVLOG 的 xwoba 條目已改為「**再次驗證 ADR §6.4，2026-07-23 的結論仍成立**」並指向 ADR。
- **補記（2026-08-03 後續）**：xwoba 票驗收時對 Savant source 做了四項修正（0 PA 守門員、
  raw 只存 tracked 列、逐年容錯、錯誤根因入訊息），連帶在 `spec-03` §3 補了一段抓取範圍與
  raw 存法的說明、`spec-01` C.7 補了 `pa > 0` 判準、`spec-03` §7 補了 `resync --season` 現在也會
  強制重抓 Savant。這些是 B 之後的新增內容，不影響 B 本身的完成度。
